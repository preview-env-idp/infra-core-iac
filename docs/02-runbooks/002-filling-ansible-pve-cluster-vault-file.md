# Runbook 002: Filling the src/ansible/group_vars/proxmox_cluster/vault.yaml file
...
```bash
cp src/ansible/inventory/group_vars/proxmox_cluster/vault.example.yaml src/ansible/inventory/group_vars/proxmox_cluster/vault.yaml
```

### 2.2. Fill the Secrets

Open `src/ansible/inventory/group_vars/proxmox_cluster/vault.yaml` in your preferred text editor. You need to replace the placeholder values with actual secrets.

* **`vault_pve_alcambic_password`**: The password for the automation service account.
...
* **`vault_pve_alcaambic_salt`**: The salt for the provided password above.
...
```bash
ansible-vault encrypt src/ansible/inventory/group_vars/proxmox_cluster/vault.yaml --vault-id pve_core@prompt
```

To verify the file is successfully encrypted, you can run `cat src/ansible/inventory/group_vars/proxmox_cluster/vault.yaml`. You should see the `$ANSIBLE_VAULT;1.2;AES256;pve_core` header instead of plaintext YAML.
...
```bash
ansible-vault decrypt src/ansible/inventory/group_vars/proxmox_cluster/vault.yaml --vault-id pve_core@prompt
```
